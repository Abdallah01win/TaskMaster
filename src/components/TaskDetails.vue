    ...
    <div class="bg-dark-300 rounded-md px-4 py-4">
        ...
        <div class="flex flex-col gap-y-2.5">
            ...
            <div class="bg-dark-200 rounded-md px-4 py-2.5">
                ...
                <input v-else v-model="form.description" type="text" class="bg-transparent focus:outline-none"
                    placeholder="Description" @keypress.enter="saveInfo" />
            </div>
            <div class="bg-dark-200 rounded-md px-4 py-2.5">
                <span v-if="task?.dueDate">{{ formatDate(task?.dueDate).date }}</span>
                <input v-else type="date" :min="minDate" v-model="form.dueDate" @change="saveInfo"
